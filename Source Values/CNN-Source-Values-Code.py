...
for env in range(4):
  ...
  for train_size in range(17):
    ...
    for iTimes in range(4):
        ...
        ModelData.append(12.5 * np.sqrt(np.sum(np.power(dis, 2))/(len(dis))))
        LossHistory.append(history.history["loss"])
        K.clear_session()

    Data.append((env, train_portion, LossHistory, ModelData))
    print(Data)

np.save("CNN Source Values Code Test Run Data5", Data)
