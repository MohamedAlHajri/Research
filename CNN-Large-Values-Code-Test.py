...
for env in range(4):
    ...
    #  all_x = np.stack([np.abs(alldata), np.angle(alldata)], axis=-1)
    #  all_x1 = np.stack([np.abs(alldata1), np.angle(alldata1)], axis=-1)
    #  all_x2 = np.stack([np.abs(alldata2), np.angle(alldata2)], axis=-1)
    #  all_x3 = np.stack([np.abs(alldata3), np.angle(alldata3)], axis=-1)
    ...
    for train_size in range(1):  # 6 #remove 4,   & make range 5
        if (train_size == 0):
        ...
        for target_size in range(start1, itera_max1):
            ...
            for iTimes in range(itera_max):  # 10 iterations
                ...
                myPred = model.predict(testData2)
                ...
                myPred = model.predict(testData3)
                ...
                # reconstructed_model = keras.models.load_model("reconstructed_model2")
                ...
                if (target_portion < 1470):
                    ...
                    model_output = reconstructed_model1.get_layer("dense_2").output
                    m = Model(inputs=reconstructed_model1.input, outputs=model_output)
                    pseudo_label_data = m.predict(trainData1_unsupervisied1)
...
for env in range(4):
    ...
    for train_size in range(1):  # 6 #remove 4,   & make range 5
        ...
        for target_size in range(start1, itera_max1):
            ...
            Data.append((env11, env22, env33, env44, env55, env66))
            print(Data)
np.save("CNN Large Values Code Test Run Data2, optimizer = adam, loss = mse", Data)
...
for env in range(4):
    ...
    for train_size in range(1):  # 6 #remove 4,   & make range 5
        ...
        for target_size in range(start1, itera_max1):
            ...
            Data.append((env11, env22, env33, env44, env55, env66))
            print(Data)
np.save("CNN Large Values Code Test Run Data3, optimizer = adam, loss = mse", Data)
